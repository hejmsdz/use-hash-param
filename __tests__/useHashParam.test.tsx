...
const buildSetterExample =
  (setterArg?, defaultValue?, setterOptions?) => () => {
    const [value, setValue] = useHashParam("value", defaultValue);
    useLayoutEffect(() => {
      setValue(setterArg, setterOptions);
    }, []);
    return <span>{value}</span>;
  };
...
const DefaultCallbackSetterExample = buildSetterExample(
  (value) => `${value}${value}`,
  "default",
);
const ResetterExample = buildSetterExample();
const EmptyStringResetterExample = buildSetterExample("");
const SetterExampleWithHistoryPush = buildSetterExample("example", undefined, {
  history: "push",
});
...
describe("useHashParam", () => {
  ...
  describe("setter function", () => {
    test("updates URL hash and variable", () => {
      ...
      return findByText("example").then(() => {
